...
| `--auto` / `--pal` / `--ntsc` | C64 clock for firmware timing and the reference. Default is `--auto`: the region is read from the SID header and a firmware named `*-pal.elf` / `*-ntsc.elf` is auto-swapped to the matching build; `--pal`/`--ntsc` force it |
